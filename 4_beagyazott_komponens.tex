...
Az osztály műkéséhez szükséges a kiolvasandó videót tartalmazó fáj elérési útja, vagy Windows operációs rendszeren egy szám, amely az azt az USB portot jelenti, amelyre a kamerát csatlakoztattuk. GNU/Linux rendszereken a kamera elérésére az -- operációs rendszer "minden fájl" szemléletének megfelelően -- a fájl elérési útját kell megadni, amely a Raspbian disztribúción a \emph{{/dev/video0}} eszköz. A működés nyomon követéséhez megadható egy \code{ILog} interfészt megfalósító objektumra, amelybe a naplóüzenetek íródnak.
