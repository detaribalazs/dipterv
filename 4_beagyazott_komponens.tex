...
Az elkészült képfeldolgozó programkomponens a \emph{cv4s} keretrendszer bármely, \code{Operation} műveletét képes folytonosan lefuttatni egy videó képkockáin. A diplomaterv egyik követelménye volt, hogy konkrét alkalmazási területre szabott feldolgozás elvégzése. A választott terület a \emph{PharmaTech} projekt gyógyszeripari gyártósor-felügyeletéhez kötődik, és feladata a vegyi anyag tömegének meghatározása kamerakép alapján. A művelet alapját küszöbözés képzi, a képfeldolgozás szempontjából nagyjából ennyiben merül ki, a további számítások ugyan nem triviálisak, de az alkalmazott platform szempontjából lényegtelenek. Mivel a félév során a tömeg meghatározását végző szoftverkomponens fejlesztési fázisban volt, így a \code{Thresholding} (küszöbözés) művelettel teszteltem az alkalmazásom működését.
...
\paragraph{\code{RealTimeProcessing}} Egy boolean típusú változó, amely igazra állítása esetén a csővezeték nem szabályozza a kameraképek kiolvasásának sebességét a terheléstől függően, hanem túlterheltség esetén hagyja elveszni a bejövő képeket.

\paragraph{\code{Reader}} ha az adott alkalmazás -- mint a beágyazott számítógépen futó, \ref{section:RpiRemoteHost} alfejezetben bemutatott program is -- a feldolgozandó képeket egy videó folyamból olvassa ki, akkor az olvasási sebesség állításához szüksége van referenciára egy, a \ref{section:CameraReader} alfejezetben bemutatott\code{CameraReader} típusú objektumra. Az osztály konstruktora úgy állítja be az objektumot, hogy a kiolvasott képek a bementi tárolóba kerüljenek. 

\paragraph{Log} a szoftvermodul a működése közben keletkező üzenetek feldolgozására szolgáló, az \code{ILog} interfészt megvalósító objektumra referenciát tárol. A többrétű felhasználás érdekében ez bármilyen, a \emph{cv4s} keretrendszer által előírt programozási felülettel rendelkező osztály példánya lehet.

\paragraph{\code{Fps}} a kiolvasási sebességet ezen a \code{unsigned int} típusú változón keresztül lehet beállítani, ez az érték csak akkor állítódik be, ha a képfeldolgozási műveletek terhelése ezt lehetővé teszi, egyébként az ahhoz legközelebbi lehetséges érték lesz. Az osztály implementálja a \code{INotifyProperty} interfészt, és a tagváltozó értékének változásáról értesítést küld a feliratkozott objektumoknak. Ez azért szükséges, hogy a feldolgozási sebesség csökkenésének esetén ez a felhasználói felületen is megjelenjen.
